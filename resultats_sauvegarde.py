...
def podium_ecuries(constructors_standings, races, constructors):
    """
    ...
    for year in sorted(fusion_finale['year'].unique(), reverse=True):
        ...
        resultats.append(f"En {year}, le podium des écuries était : {num1['name']} "
...
def nb_best_lap_time_pilote_2023(laps, races, drivers):
    ...
    return meilleur_temps_par_pilote


meilleur_temps_par_pilote = nb_best_lap_time_pilote_2023(laps, races, drivers)
# Création du graphique
fig, ax = plt.subplots(figsize=(10, 6))
...
def pilote_plus_accidents(results, status):
    """
    ...
    fusion = pd.merge(fusion, drivers[['driverId', 'nom_complet']], on='driverId')

    # Nettoyage
    fusion['status'] = fusion['status'].str.strip().str.replace('"', '', regex=False)

    # Filtrer uniquement les accidents
    fusion_accidents = fusion[fusion['status'] == "Accident"]

    if fusion_accidents.empty:
        return "Aucun accident trouvé dans les données."

    # Comptage des accidents par pilote et par année
    accidents_par_pilote = fusion_accidents.groupby(['year', 'driverId']).size().reset_index(name='nb_accidents')

    # Pilote avec le plus d'accidents chaque année
    ...
    # Génération du texte
    lignes = []
    for _, row in accidents_max.sort_values('year').iterrows():
        lignes.append(
            f"Le pilote ayant eu le plus d'accidents en {int(row['year'])} est {row['nom_complet']} "
            f"avec {int(row['nb_accidents'])} accident(s)."
        )
...
def temps_moyen_pit_stop_an(races, pit):
    """
   ...
   # Boucle sur toutes les années disponibles
    for annee in sorted(fusion['year'].unique()):
       ...
       if not pit_annee_filtre.empty:
           ...
           resultats.append(f"{annee} : {moyenne:.0f} ms")
       else:
           resultats.append(f"{annee} : Aucune donnée disponible")
...
df_victoires = victoires_par_nation(drivers, results)
    
# Création du graphique
fig, ax = plt.subplots(figsize=(10, 6))
ax.barh(df_victoires['nationality'], df_victoires['Nombre de victoires par nation'], color="#0066cc")
...
# Sauvegarde de l'image
plt.savefig(os.path.join(output_dir, "nombre_victoires_nations.png"))
plt.show()
...
def age_moyen_toutes_annees(drivers, results, races):
    """
    Calcule et retourne l'âge moyen des pilotes pour chaque année disponible.
    ...
    Returns:
    ----------
    str : texte avec l'âge moyen par année, ou un message d'information.
    """
    ...
    # Nettoyage des dates de naissance
    ...
    for annee in sorted(fusion['year'].unique()):
        ...
        if pd.isnull(age_moyen):
            resultats.append(f"{annee} : Aucun âge valide")
        else:
            resultats.append(f"{annee} : {age_moyen:.2f} ans")
...
pilotes_30_victoires = nom_pilotes_30_victoires(results, drivers)  
# Sauvegarde dans un fichier texte
with open("resultats/pilotes_30_victoires.txt", "w", encoding="utf-8") as f:
    f.write(pilotes_30_victoires)
    

# Appel de ta fonction
podium_ecuries_annees = podium_ecuries(constructors_standings, races, constructors) 
# Sauvegarde dans un fichier texte
with open("resultats/podium_ecuries_annees.txt", "w", encoding="utf-8") as f:
    f.write(podium_ecuries_annees)


# Appel de ta fonction
meilleur_temps_tour = meilleur_temps_tour_2023(laps, races)
# Sauvegarde dans un fichier texte
with open("resultats/meilleur_temps_tour_2023.txt", "w", encoding="utf-8") as f:
    f.write(meilleur_temps_tour)
    
    

# Appel de ta fonction
victoires_spa = nb_victoires_spa(results, races, drivers)
# Sauvegarde dans un fichier texte
with open("resultats/nb_victoires_spa.txt", "w", encoding="utf-8") as f:
    f.write(victoires_spa)
    
    
# Appel de ta fonction
plus_accidents = pilote_plus_accidents(results, status)
# Sauvegarde dans un fichier texte
with open("resultats/pilotes_plus_accidents.txt", "w", encoding="utf-8") as f:
    f.write(plus_accidents)
    
    
# Appel de ta fonction
temps_pit = temps_moyen_pit_stop_an(races, pit)
# Sauvegarde dans un fichier texte
with open("resultats/temps_moyen_pit_stop.txt", "w", encoding="utf-8") as f:
    f.write(temps_pit)
    
    
    
# Appel de ta fonction
circuits_plus_courus = circuit_plus_concouru(races)
# Sauvegarde dans un fichier texte
with open("resultats/circuit_plus_concouru.txt", "w", encoding="utf-8") as f:
    f.write(circuits_plus_courus)
    
    
# Appel de ta fonction
course_plus_serré = course_plus_serrée(results, races, drivers)
# Sauvegarde dans un fichier texte
with open("resultats/course_plus_serrée.txt", "w", encoding="utf-8") as f:
    f.write(course_plus_serré)
    
    
# Appel de ta fonction
age_moyen = age_moyen_toutes_annees(drivers, results, races)
# Sauvegarde dans un fichier texte
with open("resultats/age_moyen.txt", "w", encoding="utf-8") as f:
    f.write(age_moyen)